  ...
  document.querySelector("#menu").addEventListener("click", function (e) {
    ...
    switch (target) {
      ...
      case "lottery":
        ...
        if (isSup) {
          currentPrizeIndex = reservedPrizeIndex
          currentLuckys = reservedLuckys
          console.log('on end sup', currentLuckys)
          isSup = false
          saveData()
          changePrize();
          resetCard().then(res => {
      ...
      case "supplement":
        ...
        // saveData();
        if (!isSup) {
          reservedPrizeIndex = currentPrizeIndex
          reservedLuckys = currentLuckys
          currentPrizeIndex = 0
        }
        isSup = true
        resetCard().then(res => {
...
function lottery() {
  ...
  rotateBall().then(() => {
    ...
    let perCount = EACH_COUNT[currentPrizeIndex],
      ...
      console.log('剩余数量',leftCount,basicData.leftUsers)
...
function changePrize() {
  ...
  // 修改左侧prize的数目和百分比
  console.log(luckyCount, luckys, basicData.luckyUsers)
...
function changeCard(cardIndex, user) {
  ...
  card.innerHTML = `<div class="company">${COMPANY}</div><div class="name">${user[1]
    }</div><div class="details">${""}<br/>${user[2] || "PSST"}</div>`;
}
